// Apk Stock — aplikasi Android manajemen stock (konveksi).
// Nyambung ke backend FastAPI shid-konten. Login khusus admin stock.
import 'package:flutter/material.dart';
import 'package:intl/date_symbol_data_local.dart';
import 'package:provider/provider.dart';
import 'auth.dart';
import 'config.dart';
import 'services/alarm_notif.dart';
import 'screens/login_screen.dart';
import 'screens/home_shell.dart';

void main() async {
  WidgetsFlutterBinding.ensureInitialized();
  await initializeDateFormatting('id_ID', null); // format tanggal/uang Indonesia
  // Notif alarm jangan sampai bikin app gagal buka — bungkus semua di try/catch.
  try {
    await NotifAlarm.init(); // siapin channel alarm + konfigurasi service background
  } catch (e) {
    debugPrint('NotifAlarm.init gagal: $e');
  }
  final auth = AuthProvider();
  await auth.boot(); // pulihkan sesi dari token tersimpan (kalau ada)
  if (auth.isLoggedIn) {
    try {
      await NotifAlarm.start(); // sesi hidup → langsung pantau notif
    } catch (e) {
      debugPrint('NotifAlarm.start gagal: $e');
    }
  }
  runApp(ChangeNotifierProvider.value(value: auth, child: const ApkStockApp()));
}

class ApkStockApp extends StatelessWidget {
  const ApkStockApp({super.key});
  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'Andromeda Produksi',
      debugShowCheckedModeBanner: false,
      theme: AppTheme.build(),
      home: const _Gate(),
    );
  }
}

/// Gerbang: kalau sudah login → Home, kalau belum → Login.
class _Gate extends StatelessWidget {
  const _Gate();
  @override
  Widget build(BuildContext context) {
    final auth = context.watch<AuthProvider>();
    if (auth.booting) {
      return const Scaffold(body: Center(child: CircularProgressIndicator()));
    }
    return auth.isLoggedIn ? const HomeShell() : const LoginScreen();
  }
}
